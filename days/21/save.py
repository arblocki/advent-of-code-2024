    ...
    for input in inputs:
        ...
        for segment in inputSegments:
            ...
            if segment in segmentInputMap:
                segmentInputs = segmentInputMap[segment]
            else:
                for ch in segment:
            ...
            for nextInput in currentInputs:
                for nextSegment in segmentInputs:
                    newCurrentInputs.add(f'{nextInput}{nextSegment}')
...
for string in input.splitlines()[:1]:
    ...
    minLen = sys.maxsize
    # for i in range(2):
    print(f'1: {direcInputs}')
    direcInputs, minLen = getInputsForDirections(direcInputs)
    print(f'2: {direcInputs}')
    direcInputs, minLen = getInputsForDirections(direcInputs)
    print(f'3: {direcInputs}')
